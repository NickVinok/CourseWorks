...
public class ProbitFunctionsToExposureProbability {
    private static List<Double> limitValues = Stream.of(2.67, 2.95, 3.12, 3.25, 3.36, 3.45, 3.52, 3.59, 3.66,
            ...
            6.18, 6.23, 6.28, 6.34, 6.41, 6.48, 6.55, 6.64, 6.75, 6.88, 7.05,
        ...
        for(Double probit: probitValues){
            for(int j=0;j<limitValues.size();j++){
                if(probit<limitValues.get(j) && j==0){
                    ...
                    break;
                } else if(probit<limitValues.get(j)){
                    double upperLimit = limitValues.get(j);
                    double lowerLimit = limitValues.get(j-1);
                    double exactProbabilityValue;
                    if(limitValues.get(j)>7.33){
                        exactProbabilityValue = 99 + ((j+2)-100)/10d;
                    } else {
                        exactProbabilityValue = j + (probit - lowerLimit) / (upperLimit - lowerLimit);
                    }
